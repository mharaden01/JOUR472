...
A: One of the tables has the counties listed as "Kent County, Maryland" while the other has them listed as just "Kent County." If you tried to combine them, I do not think it would work, as the program would not be able to match the names up. Also, the second table does not have geoid as a section, so you couldn't join them based on that number either. 

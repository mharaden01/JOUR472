...
A: 1. One question could be: Do counties with a lower median income experience a higher amount of COVID cases per week? To answer this, you would need the data of weekly COVID cases for each county in Maryland, as well as the longitutde and latitude of each county so it can be put into a map.

Another question could be: Do lower income census tracts in PG County have a higher amount of crimes committed per year? You would need a dataset that includes the crime data and income data for each PG County Census tract as well as a dataset that contains the longitude and latitude for every tract, so the two tables can be joined together. 

...
```{r}
cdc_life_expectancy<- read_csv("CDC_Life_Census_Tract_2010_2015.csv") %>% 
  as.data.frame()
```
...
```{r}
cdc_life_expectancy <- clean_names(cdc_life_expectancy)
md_death <- filter(cdc_life_expectancy, state %in% c("Maryland"))
balt_death <- filter(md_death, county %in% c("Baltimore County, MD"))
...
**A2.** 
```{r}
balt_death2y<- read_csv("balt_death_census.csv") %>% 
  as.data.frame()
balt_death2y %>%
  summary(neighborhood$life_expectancy)
balt_death2y %>% 
  as.data.frame() %>% 
...
```{r}
md_death %>%
  summary(county$life_expectancy)
md_death %>% 
...
cdc_life_expectancy %>% 
  summarize(avg_life_ex = mean(life_expectancy, na.rm=TRUE))


```
A3: Baltimore has a slightly lower life expectancy than the national average, however it makes up many of the top communities in Maryland with the lowest life expectancy. Of the top 20 neighborhoods with the lowest life expectancy, Baltimore City is accountable for 19 of them. Their average life expectancy as a city is 73.04 years, while the state average is 78.7 years. The state itself is much closer to the national average life expectancy, 78.3 years, than Baltimore is. 
**Q4.**
...
A4: There is a strong majority of these communities that are over the average number. Surprising since Baltimore's average life expectancy is so low. 
